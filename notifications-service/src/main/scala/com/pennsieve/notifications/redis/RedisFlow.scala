...
package akka.stream.alpakka.redis.scaladsl

import java.lang
import akka.NotUsed
import akka.stream.scaladsl.Flow
import akka.stream.alpakka.redis
...
object RedisFlow {
  ...
  ): Flow[RedisKeyValue[K, V], RedisOperationResult[
    RedisKeyValue[K, V],
    String
  ], NotUsed] =
    Flow[RedisKeyValue[K, V]].mapAsync(parallelism) { redisSet =>
      val resultFuture =
        connection.async().set(redisSet.key, redisSet.value).toScala
      resultFuture
        .map(r => RedisOperationResult(redisSet, Success(r)))
        .recover {
  ...
  def mset[K, V](
    ...
    executionContext: ExecutionContext
  ): Flow[Seq[RedisKeyValue[K, V]], RedisOperationResult[Seq[
    RedisKeyValue[K, V]
  ], String], NotUsed] =
  ...
  def lpush[K, V](
    ...
    executionContext: ExecutionContext
  ): Flow[RedisKeyValues[K, V], RedisOperationResult[
    RedisKeyValues[K, V],
    Long
  ], NotUsed] =
    Flow[RedisKeyValues[K, V]].mapAsync(parallelism) { keyValues =>
      val resultFuture =
        connection.async().lpush(keyValues.key, keyValues.values: _*).toScala
      resultFuture
        .map(r => redis.RedisOperationResult(keyValues, Success(r.longValue())))
        .recover {
  ...
  def mget[K, V](
    ...
    Flow[Seq[K]].mapAsync(parallelism) { keys =>
      ...
      future
        ...
        .recover {
          case ex: Throwable => RedisOperationResult(keys, Failure(ex))
        }
    }
  ...
  def hset[K, V](
    ...
    Flow[RedisHSet[K, V]].mapAsync(parallelism) { redisHSet =>
      val future: Future[lang.Boolean] =
        connection
          .async()
          .hset(redisHSet.key, redisHSet.field, redisHSet.value)
          .toScala
      future
        .map(f => RedisOperationResult(redisHSet, Success(f)))
        .recover {
          case ex: Throwable => RedisOperationResult(redisHSet, Failure(ex))
        }
    ...
    Flow[RedisHMSet[K, V]].mapAsync(parallism) { redisHMSet =>
      val future =
        connection
          ...
          .hmset(
            redisHMSet.key,
            redisHMSet.values.map(f => f.key -> f.value).toMap.asJava
          )
          .toScala
      future
        .map(f => RedisOperationResult(redisHMSet, Success(f)))
        .recover {
          case ex: Throwable => RedisOperationResult(redisHMSet, Failure(ex))
        }
  ...
  def hdel[K, V](
    ...
    Flow[RedisHKeyFields[K]].mapAsync(parallism) { keyValues =>
      connection
        ...
        .map(
          f =>
            RedisOperationResult[RedisHKeyFields[K], Long](
              keyValues,
              Success(f)
            )
        )
