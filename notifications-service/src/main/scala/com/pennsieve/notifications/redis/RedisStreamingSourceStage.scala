...
package akka.stream.alpakka.redis.impl
...
import akka.stream.alpakka.redis.{ RedisKeyValue, RedisSubscriberSettings }
...
/**
  ...
  */
@InternalApi
trait RedisStreamingSourceStage[K, V]
    extends GraphStage[SourceShape[RedisKeyValue[K, V]]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) {
      ...
      private val buffer: util.ArrayDeque[RedisKeyValue[K, V]] =
        new java.util.ArrayDeque[RedisKeyValue[K, V]]()

      private val successCallback: AsyncCallback[RedisKeyValue[K, V]] =
        getAsyncCallback[RedisKeyValue[K, V]](handleSuccess)

      private val failureCallback: AsyncCallback[Exception] =
        getAsyncCallback[Exception](handleFailure)

      private var redisFutureCompleted = false

      val result: RedisFuture[lang.Long] = resultHandler(
        () => checkConcurrency(),
        successCallback,
        failureCallback
      )
...
/**
  ...
  */
@InternalApi
private[redis] class RedishmgetSourceStage[K, V](
  key: K,
  fields: Seq[K],
  val connection: StatefulRedisConnection[K, V],
  val settings: RedisSubscriberSettings = RedisSubscriberSettings.Defaults
          ...
          override def onKeyValue(key: K, value: V): Unit =
            if (checkConcurrency()) {
              ...
              failureCallback.invoke(
                BufferOverflowException("Redis Source buffer overflown")
              )
            }
        },
        key,
...
/**
  ...
  */
@InternalApi
private[redis] class RedishgetallSourceStage[K, V](
  val key: K,
