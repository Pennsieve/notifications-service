...
package akka.stream.alpakka.redis
...
final class RedisHMSet[K, V] private (
  val key: K,
  val values: Seq[RedisKeyValue[K, V]]
) {
  ...
  override def hashCode(): Int =
    values.map(_.hashCode()).foldLeft(0)((a, b) => 31 * a + b) + 31 * key
      .hashCode()
}
...
object RedisHMSet {
  ...
  def create[K, V](
    key: K,
    values: java.util.List[RedisKeyValue[K, V]]
  ): RedisHMSet[K, V] =
    new RedisHMSet[K, V](key, values.asScala.to[Seq])
  def apply[K, V](key: K, values: Seq[RedisKeyValue[K, V]]): RedisHMSet[K, V] =
    new RedisHMSet[K, V](key, values)
}
...
object RedisHSet {
  ...
  def apply[K, V](key: K, field: K, value: V): RedisHSet[K, V] =
    new RedisHSet(key, field, value)
  def create[K, V](key: K, field: K, value: V): RedisHSet[K, V] =
    new RedisHSet(key, field, value)
}
...
final class RedisHKeyFields[K] private (val key: K, val fields: Seq[K]) {
  ...
  override def hashCode(): Int =
    fields.map(_.hashCode()).foldLeft(0)((a, b) => 31 * a + b) + 31 + key
      .hashCode()

}

object RedisHKeyFields {
  def apply[K](key: K, fields: Seq[K]): RedisHKeyFields[K] =
    new RedisHKeyFields(key, fields)
...
object RedisPubSub {
  ...
  def apply[K, V](topic: K, value: V): RedisPubSub[K, V] =
    new RedisPubSub(topic, value)
  def create[K, V](topic: K, value: V): RedisPubSub[K, V] =
    new RedisPubSub(topic, value)
}

object RedisKeyValues {

  def apply[K, V](key: K, values: Array[V]): RedisKeyValues[K, V] =
    new RedisKeyValues(key, values)
  def create[K, V](key: K, values: Array[V]): RedisKeyValues[K, V] =
    new RedisKeyValues(key, values)
}

object RedisKeyValue {

  def apply[K, V](key: K, value: V): RedisKeyValue[K, V] =
    new RedisKeyValue(key, value)
...
final class RedisSubscriberSettings private (
  val maxConcurrency: Int,
  val maxBufferSize: Int,
  val unsubscribeOnShutDown: Boolean
) {

  def withMaxConcurrency(maxConcurrency: Int): RedisSubscriberSettings =
    copy(maxConcurrency = maxConcurrency)

  def withMaxBufferSize(maxBufferSize: Int): RedisSubscriberSettings =
    copy(maxBufferSize = maxBufferSize)

  def withUnsubscribeOnShutDown(
    unsubscribeOnShutDown: Boolean
  ): RedisSubscriberSettings =
